...
class RegistrationForm extends Model
{
	...
	public function rules()
	{
		$rules = [
			...
			[['username', 'password', 'repeat_password', 'captcha', 'email', 'name', 'surname'], 'required'],
		...
		{
			$rules[] = [['username', 'name', 'surname'], 'string', 'max' => 50];
			$rules[] = [['name', 'surname', 'email'], 'required'];
			$rules[] = ['email', 'email'];
	...
	public function registerUser($performValidation = true)
	{
		...
		if ( is_array($this->fields) ){
			...
			foreach ($this->fields as $key => $value) {
				...
				if ( ! Fields::find()->where(['name' => $field->name])->all() ){
					$field->save();
				}
